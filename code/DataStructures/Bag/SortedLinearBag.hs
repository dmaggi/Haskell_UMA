...
delete x (Node y oy s)
    | x < y             = Node y oy s       --Misma bolsa, no se toca
    | x == y && oy == 1 = s                 --Hay que eliminar el elemento, que deja de existir
    | x == y            = Node y (oy-1) s   --Ya sabemos que hay mas de 1 elemento, porque si no hubieramos entrado por arriba
...
isEmpty_insert x s = not (isEmpty (insert x s))
